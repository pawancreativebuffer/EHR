...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (configError || !config) {
      return new Response(
        JSON.stringify({ error: 'EPIC configuration not found for this client' }),
    ...
    if (req.method === 'GET') {
      ...
      const epicUrl = `${config.api_endpoint}/Patient/${patientId}`;
      ...
      if (!response.ok) {
        return new Response(
          JSON.stringify({ error: 'Failed to fetch patient from EPIC', details: await response.text() }),
          { status: response.status, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
        );
      }

      const patientData: EpicPatientResponse = await response.json();
    ...
    if (req.method === 'POST') {
      ...
      const epicUrl = `${config.api_endpoint}/Patient?${queryParams.toString()}`;
      ...
      if (!response.ok) {
        return new Response(
          JSON.stringify({ error: 'Failed to search patients in EPIC', details: await response.text() }),
          { status: response.status, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
        );
      }

      const searchResults = await response.json();
      const patients = searchResults.entry?.map((entry: { resource: EpicPatientResponse }) => entry.resource) || [];
    ...
    console.error('Error in EPIC integration:', error);
    return new Response(
